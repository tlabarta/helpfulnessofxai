...
class CEMLoss(BaseExplanationModel):
    ...
    def _init_lower_upper(
        ...
        DEFAULT_LB_UB = {
            "PP": {
                "lower": np.full(org_img.shape, np.min(org_img)),
                "upper": org_img if type(org_img) is np.ndarray else org_img.numpy(),
            },
            "PN": {
                "lower": np.where(org_img > 0, 0.0, 0.0),
                "upper": np.where(org_img < 0, 1, 0.0),
            },
        }
